...
myLayout =
    ...
    $ gaps [(U, 10), (D, 10), (R, trayerHeight + 4)] 
    $ smartBorders
    $ ifWider 2000 wide_screen narrow_screen
  where
    ...
    ratio = 1 / 3
    wide_screen = avoidFloats $ wideTopRightAngleAbsoluteLayout ||| threeColumns ||| accordionSimpleTabbed
    narrow_screen = Mirror threeColumns ||| accordionSimpleTabbed
...
setFehBackground =
    "feh --bg-fill $HOME/uc-manager/wallpapers/minimalistic-animals-deer-artwork-low-poly-1920x1080-72048.jpeg"

trayerHeight = 20 :: Int

launchTrayer =
    "trayer --edge right --align center --SetDockType true --expand true --width 30 --height " ++
    show trayerHeight ++ " --transparent true --tint 0xeae0c8 --alpha 0 --monitor primary &"
...
myKeys conf@(XConfig {XMonad.modMask = modm}) =
    ...
    , ((modm .|. controlMask, xK_r), spawn (setFehBackground ++ "; pkill xmobar; xmonad --restart"))
...
myStartupHook = do
    spawnOnce setFehBackground
    spawnOnce "xrdb $HOME/.Xresources"
    spawnOnce "xset -dpms s off"
    -- spawnOnce "xsetroot -cursor_name left_ptr"
    setDefaultCursor xC_left_ptr
    -- spawnOnce "picom --experimental-backends --config $HOME/.xmonad/picom.conf"
    spawnOnce launchTrayer
    spawnOnce "nm-applet"
    --spawnOnce "jack_control start"
...
defaults =
    def
      ...
      -- hooks, layouts
        , layoutHook = myLayout
        , manageHook = insertPosition End Newer <+> myManageHook <+> (isFullscreen --> doFullFloat) <+> manageDocks
